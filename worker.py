...
def process_separate(job: dict):
    """스템 분리: 원곡 다운로드 → demucs(CPU) → 4개 mp3 업로드."""
    qid, track_id = job["queue_id"], job["track_id"]
    log.info("스템 분리 시작: track=%s", track_id)
    if not Path(ML_PYTHON).exists():
        log.error("ML 파이썬 없음: %s (mlenv에 demucs 설치 필요)", ML_PYTHON)
        _report_fail(qid, "워커에 분리 환경(mlenv)이 없습니다.")
        return
    started = time.time()
    with tempfile.TemporaryDirectory() as tmp:
        tmp = Path(tmp)
        ...
        try:
            with httpx.Client(timeout=300) as client:
                with client.stream("GET", f"{RAILWAY_URL}/api/audio/{track_id}") as r:
                    r.raise_for_status()
                    with open(src, "wb") as f:
                        for chunk in r.iter_bytes():
                            f.write(chunk)
        except Exception as e:
            log.error("원곡 다운로드 실패 (%s): %s", track_id, e)
            _report_fail(qid, f"원곡 다운로드 실패: {e}")
            return

        out_dir = tmp / "out"
        r = subprocess.run(
            [ML_PYTHON, "-m", "demucs", "-d", "cpu", "-n", DEMUCS_MODEL,
             "--mp3", "--mp3-bitrate", "192", "-o", str(out_dir), str(src)],
            capture_output=True, text=True, timeout=3600,
        )
        if r.returncode != 0:
            tail = (r.stderr or r.stdout or "")[-500:]
            log.error("demucs 실패 (track=%s): %s", track_id, tail)
            _report_fail(qid, f"분리 실패: {tail[-200:]}")
            return

        stem_dir = out_dir / DEMUCS_MODEL / src.stem
        paths = {s: stem_dir / f"{s}.mp3" for s in STEM_NAMES}
        missing = [s for s, p in paths.items() if not p.exists()]
        if missing:
            log.error("스템 파일 누락 (track=%s): %s", track_id, missing)
            _report_fail(qid, f"분리 결과 누락: {missing}")
            return

        try:
            handles = {s: open(p, "rb") for s, p in paths.items()}
            try:
                resp = httpx.post(
            ...
            if resp.status_code == 410:
                log.info("취소됨 (사용자가 대기열에서 취소): track=%s", track_id)
                return
            resp.raise_for_status()
        except Exception as e:
            log.error("스템 업로드 실패 (track=%s): %s", track_id, e)
            _report_fail(qid, f"스템 업로드 실패: {e}")
            return
    log.info("스템 분리 완료: track=%s (%.0f초)", track_id, time.time() - started)
...
def main():
    ...
    while True:
        ...
        job_type = job.get("type", "extract")
        if job_type == "separate" and job.get("track_id"):
            process_separate(job)
        elif job.get("url"):
            process(job)
        else:
